...
{
    internal class AlphaBeta : ISearchAlgorithm
    {
        ...
        public AlphaBeta(Player player, GameState game, IStrategyReversi strategy, IStrategyReversi endgameStrategy, int depth, int endgameStart = 44)
        {
            MyPlayer = player;
            Game = game;
            Strategy = strategy;
        ...
        public Position FindMove(Player player, GameState gameState, bool IsRandom=false)
        {
            ...
            foreach (var move in gameState.LegalMoves.Keys)
            {
                ...
                if (IsRandom)
                {
                    ...
                    legalValues[move] = FindMoveRec(player.Opponent(), afterMove, Depth - 1, alpha, beta);
        ...
        //player - kogo tura
        public double FindMoveRec(Player player, GameState gameState, int curDepth, double alpha, double beta)
        {
            NodesSearchedCount++;
            ...
            {
                return FindMoveRec(player.Opponent(), gameState, curDepth - 1, alpha, beta);
            }
            ...
            if (player == MyPlayer)
            {
                ...
                foreach (var move in gameState.LegalMoves.Keys)
                {
                    ...
                    afterMove.MakeMove(move);
                    value = Math.Max(value, FindMoveRec(player.Opponent(), afterMove, curDepth - 1, alpha, beta));
            ...
            else
            {
                ...
                foreach (var move in gameState.LegalMoves.Keys)
                {
                    GameState afterMove = new GameState(gameState);
                    afterMove.MakeMove(move);
                    value = Math.Min(value, FindMoveRec(player.Opponent(), afterMove, curDepth - 1, alpha, beta));
